...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        for curLayer in [model.last.weight,model.last.bias]: #here we only apply MaxNorm over the last two layers
    ...
    def PGD(self, model):
        ...
        for i, curLayer in enumerate([model.last.weight,model.last.bias]): #here we only apply MaxNorm over the last two layers
            curparam = curLayer.data
            ...
            #print("current parametner : ",curparam)
class Normalizer(): 
    ...
    def apply_on(self, model): #this method applies tau-normalization on the classifier layer
        ...
        for curLayer in [model.last.weight]: #change to last layer: Done
            curparam = curLayer.data
