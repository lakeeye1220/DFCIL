...
class ISCF(NormalNN):
        ...
        if need_train:
            ...
            if ft_flag is False:
                ...
                for module in self.model.modules():
                    ...
                    #print("bntrain datafree.ly 422 module requires_grad ",module.weight.requires_grad)
                '''
                for param in self.model.parameters():
                ...
                '''
                #print("After freezing, model print",self.model)
                ...
                for epoch in range(self.config['finetune_epoch']):
                    ...
                    for i, (x, y, task)  in enumerate(train_loader):
                        ...
                        # model update
                        loss, loss_finetune, output= self.finetune_model(x_com, y_com, y_hat_com, dw_force = dw_cls, kd_index = np.arange(len(x), len(x_com)))
                        # measure elapsed time
                        batch_time.update(batch_timer.toc()) 
    ...
    def finetune_model(self, inputs, targets, target_scores = None, dw_force = None, kd_index = None):
        # forward pass
        #print(module.weight[0].requires_grad)
        task_step=self.valid_out_dim-self.last_valid_out_dim
        # class balancing
        ...
        if self.inversion_replay:
            # local classification - LCE loss: the logit dimension is from last_valid_out_dim to valid_out_dim
            loss_finetune = self.criterion(logits[class_idx,self.last_valid_out_dim:self.valid_out_dim], (targets[class_idx]-self.last_valid_out_dim).long(), dw_cls[class_idx]) 
            
            # ft classification  
            if len(self.config['gpuid']) > 1:
                loss_finetune += self.criterion(self.model.module.last(logits_pen.detach()), targets.long(), dw_cls)
            else:
                loss_finetune += self.criterion(self.model.last(logits_pen.detach()), targets.long(), dw_cls)
        
        #first task local classification when we do not use any synthetic data     
        else:
            #loss_finetune = torch.zeros((1,),requires_grad=True).cuda()
            loss_finetune = self.criterion(logits[class_idx], targets[class_idx].long(), dw_cls[class_idx])
        ...
        return total_loss.detach(), loss_finetune.detach(),logits