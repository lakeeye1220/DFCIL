...
class ISCF(NormalNN):
    ...
    def update_model(self, inputs, targets,target_scores = None, dw_force = None, kd_index = None):
        ...
        if self.inversion_replay:
            ...
            loss_class = self.criterion(self.model.last(r_logits_pen)[class_idx,self.last_valid_out_dim:self.valid_out_dim],(targets[class_idx]-self.last_valid_out_dim).long(),dw_cls[class_idx])
            # ft classification 
            ...
            if len(self.config['gpuid']) > 1:
                ...
                loss_class += self.large_margin_criterion(self.model.module.last(f_logits_pen.detach()), one_hot[fake_class_idx], f_m, dw_cls[fake_class_idx])
        ...
        if self.previous_teacher: # after 2nd task
            ...
            if len(pm)==3:
                ...
                out1_rm,out2_rm,out3_rm=r_m
                out1_fm,out2_fm,out3_fm=f_m
                out1_m = torch.cat([out1_rm,out1_fm],dim=0)
                out2_m = torch.cat([out2_rm,out2_fm],dim=0)
                out3_m = torch.cat([out3_rm,out3_fm],dim=0)
        ...
        if self.previous_teacher:
            ...
            logits = self.model.last(torch.cat([r_logits_pen,f_logits_pen],dim=0))
        ...
        total_loss = loss_class + loss_sp + loss_lkd + weq_regularizer 
